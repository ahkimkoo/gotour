...
var (
	...
	dict         = flag.String("dict", "github.com/huichen/sego/data/dictionary.txt", "词典文件")
...
func TextRpcServer(w http.ResponseWriter, req *http.Request) {
	// 得到要分词的文本
	text := req.URL.Query().Get("text")
	mark := req.URL.Query().Get("mark")
	if text == "" {
		text = req.PostFormValue("text")
	}
	if mark == "" {
	...
	ss := []string{}
	for _, segment := range segments {
		ss = append(ss, segment.Token().Text())
	}

	w.Header().Set("Content-Type", "application/json")
	io.WriteString(w, strings.Join(ss, mark)+mark)
}
...
func main() {
	...
	http.HandleFunc("/text", TextRpcServer)
